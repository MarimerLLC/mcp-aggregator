...
Some downstream servers expose prompt templates — reusable message sequences that can be parameterized. Each one is exposed by the aggregator as a real MCP prompt named `{server}__{prompt}` (for example `my-server__summarize_document`) with the downstream prompt's own arguments, so a client that supports prompts lists and renders them the ordinary way (`prompts/list`, `prompts/get`). `find_tools` returns matching prompts under `prompts`, and `get_service_details` returns every prompt of a server with its `wrapperName` and `arguments` (required vs. optional).

**Example:**
```
find_tools(query: "summarize document")
  → prompts: [{ prompt: "my-server__summarize_document", server: "my-server",
                arguments: [{ name: "document", required: true }, { name: "style", required: false }], activated: true }]

prompts/get(name: "my-server__summarize_document", arguments: { document: "...content...", style: "bullet-points" })
```

The result contains a description and a list of messages (with role and content) ready to inject into a conversation. A prompt requested without a required argument fails with an error naming the argument; the downstream is not contacted.

`WrapperMode` applies to prompts as it does to tools: in `Lazy` mode a prompt appears in **your** prompt list after `find_tools` / `get_service_details` (the aggregator sends `prompts/list_changed`), and any existing `{server}__{prompt}` name is accepted by `prompts/get` whether or not it is listed; in `Eager` mode every prompt of every enabled server is always listed.

**Fallback.** If your client has no prompt support, call `get_prompt` with the server name, the downstream prompt name, and the arguments as a JSON object encoded as a string:
